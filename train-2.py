...
class NeuralNetwork(torch.nn.Module):
    def __init__(self, inputs, output, hidden_units, drop)  #  neural network hidden units
        super().__init__() # calls init method of nn.Module (base class)
        self.hidden_units = nn.Module([nn.Linear(inputs, hidden_units[0])]) # input_size to hidden_units 1 : Module --> list meant to store nn.Modules
        
        # add random number of hidden units
        i = 0
        j = len(hidden_units)-1
        
        while i != j:
            l = [hidden_units[i], hidden_units[i+1]]
            self.hidden_units.append(nn.Linear(l[0], l[1]))
            i+=1

        # check hidden units are process ok
        for each in hidden_units:
            print(each)
        
        # last hidden units -> output
        self.output = nn.Linear(hidden_units[j], output)
...
# feedforward method    
    def forward(self, tensor):
        ...
        for linear in self.hidden_units:
